local bit = require "bit"
local JSON = require "cjson"
local VERSION = 2
local KEY_VERSION = "version"
local DEVICE_TYPE = 0xB1
local DEVICE_CONTROL_REQUEST = 0x02
local DEVICE_QUERY_REQUEST = 0x03
local DEVICE_PROTOCOL_HEAD = 0xAA
local DEVICE_PROTOCOL_LENGTH = 0x0A
local POWER_ON = 0x01
local POWER_OFF = 0x07
local WORK_STATUS_NONE = 0xFF
local WORK_STATUS_CANCEL = 0x01
local WORK_STATUS_WORK = 0x02
local WORK_STATUS_PAUSE = 0x03
local WORK_STATUS_END = 0x04
local WORK_STATUS_APPOINTMENT = 0x06
local MODE_KEEP_WARM = 0x01;
local MODE_STERO_BAKING = 0x02;
local MODE_WHOLE_BAKING = 0x03;
local MODE_UP_DOWN_BAKING = 0x04;
local MODE_UNFREEZE = 0x05;
local MODE_HOTAIR_CONVECTION = 0x06;
local MODE_POWER_SAVING = 0x07;
local MODE_CENTER_BAKING = 0x08;
local MODE_ROTARY_BAKING = 0x09;
local LOCK_ON = 0x01;
local LOCK_OFF = 0x00;
local KEY_POWER = "power"
local KEY_WORK_STATUS = "work_status"
local KEY_MODE = "mode"
local KEY_ERROR_CODE = "error_code"
local KEY_LOCK = "lock"
local KEY_HOUR = "hour"
local KEY_MINUTES = "minutes"
local KEY_TEMPERATURE = "temperature"
local VALUE_ON = "on"
local VALUE_OFF = "off"
local VALUE_WORK_STATUS_NONE = "none"
local VALUE_WORK_STATUS_CANCEL = "cancel"
local VALUE_WORK_STATUS_WORK = "work"
local VALUE_WORK_STATUS_PAUSE = "pause"
local VALUE_WORK_STATUS_END = "end"
local VALUE_WORK_STATUS_APPOINTMENT = "appointment"
local VALUE_MODE_KEEP_WARM = "keep_warm"
local VALUE_MODE_STERO_BAKING = "stero_baking"
local VALUE_MODE_WHOLE_BAKING = "whole_baking"
local VALUE_MODE_UP_DOWN_BAKING = "up_down_baking"
local VALUE_MODE_UNFREEZE = "unfreeze"
local VALUE_MODE_HOTAIR_CONVECTION = "hot_air_convection"
local VALUE_MODE_POWER_SAVING = "power_saving"
local VALUE_MODE_CENTER_BAKING = "center_baking"
local VALUE_MODE_ROTARY_BAKING = "rotary_baking"
local workstatus
local mode
local hour
local minutes
local temperature
local curTemp
local lock
local errorCode
local dataType = 0
function updateGlobalPropertyValueByJson(luaTable)
    if luaTable[KEY_POWER] == VALUE_ON then
        workstatus = POWER_ON
    elseif luaTable[KEY_POWER] == VALUE_OFF then
        workstatus = POWER_OFF
    end
    if luaTable[KEY_WORK_STATUS] == VALUE_WORK_STATUS_NONE then
        workstatus = WORK_STATUS_NONE
    elseif luaTable[KEY_WORK_STATUS] == VALUE_WORK_STATUS_CANCEL then
        workstatus = WORK_STATUS_CANCEL
    elseif luaTable[KEY_WORK_STATUS] == VALUE_WORK_STATUS_WORK then
        workstatus = WORK_STATUS_WORK
    elseif luaTable[KEY_WORK_STATUS] == VALUE_WORK_STATUS_PAUSE then
        workstatus = WORK_STATUS_PAUSE
    end
    if luaTable[KEY_MODE] == VALUE_MODE_KEEP_WARM then
        mode = MODE_KEEP_WARM
    elseif luaTable[KEY_MODE] == VALUE_MODE_STERO_BAKING then
        mode = MODE_STERO_BAKING
    elseif luaTable[KEY_MODE] == VALUE_MODE_WHOLE_BAKING then
        mode = MODE_WHOLE_BAKING
    elseif luaTable[KEY_MODE] == VALUE_MODE_UP_DOWN_BAKING then
        mode = MODE_UP_DOWN_BAKING
    elseif luaTable[KEY_MODE] == VALUE_MODE_UNFREEZE then
        mode = MODE_UNFREEZE
    elseif luaTable[KEY_MODE] == VALUE_MODE_HOTAIR_CONVECTION then
        mode = MODE_HOTAIR_CONVECTION
    elseif luaTable[KEY_MODE] == VALUE_MODE_POWER_SAVING then
        mode = MODE_POWER_SAVING
    elseif luaTable[KEY_MODE] == VALUE_MODE_CENTER_BAKING then
        mode = MODE_CENTER_BAKING
    elseif luaTable[KEY_MODE] == VALUE_MODE_ROTARY_BAKING then
        mode = MODE_ROTARY_BAKING
    end
    if luaTable[KEY_LOCK] == VALUE_ON then
        lock = LOCK_ON
    elseif luaTable[KEY_LOCK] == VALUE_OFF then
        lock = LOCK_OFF
    end
    if luaTable[KEY_HOUR] ~= nil then hour = luaTable[KEY_HOUR] end
    if luaTable[KEY_MINUTES] ~= nil then minutes = luaTable[KEY_MINUTES] end
    if luaTable[KEY_TEMPERATURE] ~= nil then
        temperature = luaTable[KEY_TEMPERATURE]
    end
end
function updateGlobalPropertyValueByByte(messageBytes)
    if #messageBytes >= 16 then
        workstatus = messageBytes[0]
        mode = messageBytes[1]
        hour = messageBytes[2]
        minutes = messageBytes[3]
        if messageBytes[4] < 0 then
            curTemp = messageBytes[4] + 256
        else
            curTemp = messageBytes[4]
        end
        errorCode = messageBytes[5]
        if messageBytes[8] < 0 then
            temperature = (messageBytes[8] + 256);
        else
            temperature = messageBytes[8];
        end
        lock = messageBytes[16];
    end
end
function assembleJsonByGlobalProperty()
    local streams = {}
    streams[KEY_VERSION] = VERSION
    if (workstatus == POWER_ON) then
        streams[KEY_POWER] = VALUE_ON
    elseif (workstatus == POWER_OFF) then
        streams[KEY_POWER] = VALUE_OFF
    end
    if (workstatus == WORK_STATUS_NONE) then
        streams[KEY_WORK_STATUS] = VALUE_WORK_STATUS_NONE
    elseif (workstatus == WORK_STATUS_CANCEL) then
        streams[KEY_WORK_STATUS] = VALUE_WORK_STATUS_CANCEL
    elseif workstatus == WORK_STATUS_WORK then
        streams[KEY_WORK_STATUS] = VALUE_WORK_STATUS_WORK
    elseif workstatus == WORK_STATUS_PAUSE then
        streams[KEY_WORK_STATUS] = VALUE_WORK_STATUS_PAUSE
    elseif workstatus == WORK_STATUS_END then
        streams[KEY_WORK_STATUS] = VALUE_WORK_STATUS_END
    elseif workstatus == WORK_STATUS_APPOINTMENT then
        streams[KEY_WORK_STATUS] = VALUE_WORK_STATUS_APPOINTMENT
    end
    if (mode == MODE_KEEP_WARM) then
        streams[KEY_MODE] = VALUE_MODE_KEEP_WARM
    elseif (mode == MODE_STERO_BAKING) then
        streams[KEY_MODE] = VALUE_MODE_STERO_BAKING
    elseif (mode == MODE_WHOLE_BAKING) then
        streams[KEY_MODE] = VALUE_MODE_WHOLE_BAKING
    elseif (mode == MODE_UP_DOWN_BAKING) then
        streams[KEY_MODE] = VALUE_MODE_UP_DOWN_BAKING
    elseif (mode == MODE_UNFREEZE) then
        streams[KEY_MODE] = VALUE_MODE_UNFREEZE
    elseif (mode == MODE_HOTAIR_CONVECTION) then
        streams[KEY_MODE] = VALUE_MODE_HOTAIR_CONVECTION
    elseif (mode == MODE_POWER_SAVING) then
        streams[KEY_MODE] = VALUE_MODE_POWER_SAVING
    elseif (mode == MODE_CENTER_BAKING) then
        streams[KEY_MODE] = VALUE_MODE_CENTER_BAKING
    elseif (mode == MODE_ROTARY_BAKING) then
        streams[KEY_MODE] = VALUE_MODE_ROTARY_BAKING
    end
    streams[KEY_ERROR_CODE] = errorCode
    if (lock == LOCK_ON) then
        streams[KEY_LOCK] = VALUE_ON
    elseif (lock == LOCK_OFF) then
        streams[KEY_LOCK] = VALUE_OFF
    end
    streams[KEY_HOUR] = hour
    streams[KEY_MINUTES] = minutes
    streams[KEY_TEMPERATURE] = temperature
    return streams
end
function jsonToData(jsonCmdStr)
    if (#jsonCmdStr == 0) then return nil end
    local msgBytes
    local json = decodeJsonToTable(jsonCmdStr)
    local deviceSubType = json["deviceinfo"]["deviceSubType"]
    if (deviceSubType == 1) then end
    local query = json["query"]
    local control = json["control"]
    local status = json["status"]
    if (control) then
        if (status) then updateGlobalPropertyValueByJson(status) end
        if (control) then updateGlobalPropertyValueByJson(control) end
        local bodyLength = 15
        local bodyBytes = {}
        for i = 0, bodyLength - 1 do bodyBytes[i] = 0 end
        bodyBytes[0] = workstatus
        bodyBytes[1] = mode
        bodyBytes[2] = hour
        bodyBytes[3] = minutes
        bodyBytes[4] = temperature
        bodyBytes[10] = lock
        msgBytes = assembleUart(bodyBytes, DEVICE_CONTROL_REQUEST)
    elseif (query) then
        local bodyLength = 1
        local bodyBytes = {}
        for i = 0, bodyLength - 1 do bodyBytes[i] = 0 end
        msgBytes = assembleUart(bodyBytes, DEVICE_QUERY_REQUEST)
    end
    local infoM = {}
    local length = #msgBytes + 1
    for i = 1, length do infoM[i] = msgBytes[i - 1] end
    local ret = table2string(infoM)
    ret = string2hexstring(ret)
    return ret
end
function dataToJson(jsonStr)
    if (not jsonStr) then return nil end
    local json = decodeJsonToTable(jsonStr)
    local deviceinfo = json["deviceinfo"]
    local deviceSubType = deviceinfo["deviceSubType"]
    if (deviceSubType == 1) then end
    local binData = json["msg"]["data"]
    local status = json["status"]
    if (status) then updateGlobalPropertyValueByJson(status) end
    local bodyBytes = {}
    local byteData = string2table(binData)
    dataType = byteData[10];
    bodyBytes = extractBodyBytes(byteData)
    local ret = updateGlobalPropertyValueByByte(bodyBytes)
    local retTable = {}
    retTable["status"] = assembleJsonByGlobalProperty()
    local ret = encodeTableToJson(retTable)
    return ret
end
function extractBodyBytes(byteData)
    local msgLength = #byteData
    local msgBytes = {}
    local bodyBytes = {}
    for i = 1, msgLength do msgBytes[i - 1] = byteData[i] end
    local bodyLength = msgLength - DEVICE_PROTOCOL_LENGTH - 1
    for i = 0, bodyLength - 1 do
        bodyBytes[i] = msgBytes[i + DEVICE_PROTOCOL_LENGTH]
    end
    return bodyBytes
end
function assembleUart(bodyBytes, type)
    local bodyLength = #bodyBytes + 1
    if bodyLength == 0 then return nil end
    local msgLength = (bodyLength + DEVICE_PROTOCOL_LENGTH + 1)
    local msgBytes = {}
    for i = 0, msgLength - 1 do msgBytes[i] = 0 end
    msgBytes[0] = DEVICE_PROTOCOL_HEAD
    msgBytes[1] = msgLength - 1
    msgBytes[2] = DEVICE_TYPE
    msgBytes[9] = type
    for i = 0, bodyLength - 1 do
        msgBytes[i + DEVICE_PROTOCOL_LENGTH] = bodyBytes[i]
    end
    msgBytes[msgLength - 1] = makeSum(msgBytes, 1, msgLength - 2)
    return msgBytes
end
function makeSum(tmpbuf, start_pos, end_pos)
    local resVal = 0
    for si = start_pos, end_pos do
        resVal = resVal + tmpbuf[si]
        if resVal > 0xff then resVal = bit.band(resVal, 0xff) end
    end
    resVal = 255 - resVal + 1
    return resVal
end
local crc8_854_table = {
    0, 94, 188, 226, 97, 63, 221, 131, 194, 156, 126, 32, 163, 253, 31, 65, 157,
    195, 33, 127, 252, 162, 64, 30, 95, 1, 227, 189, 62, 96, 130, 220, 35, 125,
    159, 193, 66, 28, 254, 160, 225, 191, 93, 3, 128, 222, 60, 98, 190, 224, 2,
    92, 223, 129, 99, 61, 124, 34, 192, 158, 29, 67, 161, 255, 70, 24, 250, 164,
    39, 121, 155, 197, 132, 218, 56, 102, 229, 187, 89, 7, 219, 133, 103, 57,
    186, 228, 6, 88, 25, 71, 165, 251, 120, 38, 196, 154, 101, 59, 217, 135, 4,
    90, 184, 230, 167, 249, 27, 69, 198, 152, 122, 36, 248, 166, 68, 26, 153,
    199, 37, 123, 58, 100, 134, 216, 91, 5, 231, 185, 140, 210, 48, 110, 237,
    179, 81, 15, 78, 16, 242, 172, 47, 113, 147, 205, 17, 79, 173, 243, 112, 46,
    204, 146, 211, 141, 111, 49, 178, 236, 14, 80, 175, 241, 19, 77, 206, 144,
    114, 44, 109, 51, 209, 143, 12, 82, 176, 238, 50, 108, 142, 208, 83, 13,
    239, 177, 240, 174, 76, 18, 145, 207, 45, 115, 202, 148, 118, 40, 171, 245,
    23, 73, 8, 86, 180, 234, 105, 55, 213, 139, 87, 9, 235, 181, 54, 104, 138,
    212, 149, 203, 41, 119, 244, 170, 72, 22, 233, 183, 85, 11, 136, 214, 52,
    106, 43, 117, 151, 201, 74, 20, 246, 168, 116, 42, 200, 150, 21, 75, 169,
    247, 182, 232, 10, 84, 215, 137, 107, 53
}
function crc8_854(dataBuf, start_pos, end_pos)
    local crc = 0
    for si = start_pos, end_pos do
        crc = crc8_854_table[bit.band(bit.bxor(crc, dataBuf[si]), 0xFF) + 1]
    end
    return crc
end
function decodeJsonToTable(cmd)
    local tb
    if JSON == nil then JSON = require "cjson" end
    tb = JSON.decode(cmd)
    return tb
end
function encodeTableToJson(luaTable)
    local jsonStr
    if JSON == nil then JSON = require "cjson" end
    jsonStr = JSON.encode(luaTable)
    return jsonStr
end
function string2table(hexstr)
    local tb = {}
    local i = 1
    local j = 1
    for i = 1, #hexstr - 1, 2 do
        local doublebytestr = string.sub(hexstr, i, i + 1)
        tb[j] = tonumber(doublebytestr, 16)
        j = j + 1
    end
    return tb
end
function string2hexstring(str)
    local ret = ""
    for i = 1, #str do ret = ret .. string.format("%02x", str:byte(i)) end
    return ret
end
function table2string(cmd)
    local ret = ""
    local i
    for i = 1, #cmd do ret = ret .. string.char(cmd[i]) end
    return ret
end
function checkBoundary(data, min, max)
    if (not data) then data = 0 end
    data = tonumber(data)
    if ((data >= min) and (data <= max)) then
        return data
    else
        if (data < min) then
            return min
        else
            return max
        end
    end
end
function print_lua_table(lua_table, indent)
    indent = indent or 0
    for k, v in pairs(lua_table) do
        if type(k) == "string" then k = string.format("%q", k) end
        local szSuffix = ""
        if type(v) == "table" then szSuffix = "{" end
        local szPrefix = string.rep("    ", indent)
        formatting = szPrefix .. "[" .. k .. "]" .. " = " .. szSuffix
        if type(v) == "table" then
            print(formatting)
            print_lua_table(v, indent + 1)
            print(szPrefix .. "},")
        else
            local szValue = ""
            if type(v) == "string" then
                szValue = string.format("%q", v)
            else
                szValue = tostring(v)
            end
            print(formatting .. szValue .. ",")
        end
    end
end
