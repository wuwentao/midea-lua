...
local function getChannels(inputChannels)
    local channels = {}
    for channel in string.gmatch(inputChannels, "%d+") do
        -- 将提取的数字转换为数值类型并插入到表中
...
local function remote_control_info(table1, messageBytes, cursor, init_pos,
                                   byteNum)
    local i = 0
    for key, value in pairs(table1) do
        if (i >= byteNum) then break end
        for k, v in pairs(value) do
            if (v == 1) then
                keyP[k] = messageBytes[cursor + init_pos + i]
                i = i + 1
            end
            if (v == 2) then
                if k == "choose_resonance_point" or k == "choose_value" then
                    keyP[k] = messageBytes[cursor + init_pos + i]
                    i = i + 1
                end
            end
...
local function remote_control_info_channel_0e(table1, messageBytes, cursor,
                                              init_pos, byteNum)
    local i = 0
    if (i < byteNum) then
        -- 首先需要判断 当前有多少个定制
        keyP["unchange_value_channel_0e"] = messageBytes[cursor + 3]
        cursor = cursor + 3
        for i = 1, keyP["unchange_value_channel_0e"] do
            keyP["channel_" .. i] = messageBytes[cursor + 1]
            keyP["channel_value_" .. i] = messageBytes[cursor + 2]
            cursor = cursor + 2
        end
    end
    return cursor
...
local function remote_control_result(table1, messageBytes, cursor, init_pos)
    for key, value in pairs(table1) do
        for k, v in pairs(value) do
            if (v == 1) then
                keyP[k] = messageBytes[cursor + init_pos + 1]
                cursor = cursor + 1
            end
            if (v == 2) then
                if k == "choose_resonance_point" or k == "choose_value" then
...
local function remote_query_info(table1, messageBytes, cursor, init_pos, byteNum)
    local i = 0
    for key, value in pairs(table1) do
        if (i >= byteNum) then break end
        for k, v in pairs(value) do
            ...
            if (v == 2) then
                if (k == "ac_type" or k == "electric_current_value" or k ==
                    "outdoor_voltage_effective_value" or k ==
                    "ac_indoor_capability_status") then
                    -- 条件为真时执行的代码
                    keyP[k] = messageBytes[cursor + init_pos + i]
                    i = i + 1
                elseif (k == "outdoor_fan_setting_speed" or k ==
                    "electronic_expansion_open" or k ==
                    "outdoor_fan_actual_speed") then
                    keyP[k] = (messageBytes[cursor + init_pos + i + 1] +
                                  bit.lshift(
                                      messageBytes[cursor + init_pos + i], 8))
                    i = i + 2
                else
                    keyP[k] = (messageBytes[cursor + init_pos + i + 1] +
                                  bit.lshift(
...
local function calculateBitMask(remoteChannels, startRange, endRange)
    local bitMask = 0
    for _, channel in ipairs(remoteChannels) do
        if channel >= startRange and channel <= endRange then
            -- 将对应的位设置为 1
            bitMask = bit.bor(bitMask, bit.lshift(1, 31 - channel))
        end
    end
    return bitMask
end

local function extractBytes(bitMask, startIndex, bodyBytes)
    for i = 3, 0, -1 do
        local byteValue = bit.band(bit.rshift(bitMask, (i * 8)), 0xFF)

        -- 提取每个字节并转换为十六进制字符串
        bodyBytes[startIndex + 3 - i] = byteValue
    end
end
...
-- 00dd 0103构造bodyBytes方法
local function build_bodyBytes_new(table1, bodyBytes, cursor)
    local i = 0
    for key, value in pairs(table1) do
        for k, v in pairs(value) do
            if (v == 1 and keyP[k] ~= nil) then
                bodyBytes[cursor + i] = keyP[k]
                i = i + 1
            end
        end
    end
...
-- t1 温度
local T1_temperature_01 = {
    {["T1_temperature"] = 2} -- t1
}
-- t2 温度
local T2_temperature_02 = {
    {["T2_temperature"] = 2} -- t2
}
-- t3 温度
local T3_temperature_03 = {
    {["T3_temperature"] = 2} -- t3
}
-- t4 温度
local T4_temperature_04 = {
    {["T4_temperature"] = 2} -- t4
}
-- t5 温度
local T5_temperature_05 = {
    {["T5_temperature"] = 2} -- t5
}

-- 压缩机目标频率
local compressor_target_frequency_06 = {
    {["compressor_target_frequency"] = 1} -- 压缩机目标频率
}
-- 压缩机实际运行频率
local compressor_actual_frequency_07 = {
    {["compressor_actual_frequency"] = 1} -- 压缩机目标频率
}
-- 空调参数
local ac_parameter_08 = {
    {["ac_type"] = 1}, -- 0：变频机，1：定频机
    {["electric_current_value"] = 1} -- 变频机：外机总电流值（单位：A），单位：A；定频机：压缩机电流AD值
}
-- 室内机参数
local indoor_machine_parameter_09 = {
    {["outdoor_voltage_effective_value"] = 1} -- 室外电压有效值（单位：V）
}

-- 10 室内能力测试状态
local indoor_machine_parameter_0a = {
    {["ac_indoor_capability_status"] = 1} -- bit0：快检；bit1：强劲；bit2：睡眠；bit3：强制自动；bit[4:7]:1：额定能力测试；4：中间能力测试；5：25%能力测试；8：最大能力测试；12：最小能力测试；其它预留
}

-- 11
local indoor_running_mode_0b = {
    {["indoor_running_mode"] = 1} -- bit0：快检；bit1：强劲；bit2：睡眠；bit3：强制自动；bit[4:7]:1：额定能力测试；4：中间能力测试；5：25%能力测试；8：最大能力测试；12：最小能力测试；其它预留
}

-- 12 室外风机设定转速
local outdoor_fan_setting_speed_0c = {
    {["outdoor_fan_setting_speed"] = 2} -- 室外风机设定转速
}

-- 13 电子膨胀阀开度
local electronic_expansion_open_0d = {
    {["electronic_expansion_open"] = 2} -- 电子膨胀阀开度高字节
}
-- 14 室外风机实际转速
local outdoor_fan_actual_speed_0e = {
    {["outdoor_fan_actual_speed"] = 2} -- 室外风机设定转速
}
-- 15 室内湿度
local indoor_humidity_0f = {
    {["indoor_humidity"] = 1} -- 室内湿度
}

-- 16 补偿温度
local compensate_temperature_10 = {
    {["compensate_temperature"] = 2} -- 补偿温度
}
local not_support_11 = {
    {["not_support"] = 0} -- 一当前不支持
}
-- 18 wifi信号强度
local wifi_singal_strength_12 = {
    {["wifi_singal_strength"] = 1} -- wifi信号强度
}
-- 19 直流母线电压
local DC_bus_voltage_13 = {
    {["DC_bus_voltage"] = 1} -- wifi信号强度
}

-- 20 室内目标频率
local indoor_target_frequency_14 = {
    {["indoor_target_frequency"] = 1} -- 室内目标频率
}

-- 21 T2A温度
local T2A_temperature_15 = {
    {["T2A_temperature"] = 1} -- 室内目标频率
}
-- 22 T2B温度
local T2B_temperature_16 = {
    {["T2B_temperature"] = 1} -- 室内目标频率
}

local not_support_17 = {
    {["not_support"] = 0} -- 一当前不支持
}
local not_support_18 = {
    {["not_support"] = 0} -- 一当前不支持
}
local not_support_19 = {
    {["not_support"] = 0} -- 一当前不支持
}
local not_support_1a = {
    {["not_support"] = 0} -- 一当前不支持
}
local not_support_1b = {
    {["not_support"] = 0} -- 一当前不支持
}
local not_support_1c = {
    {["not_support"] = 0} -- 一当前不支持
}
local not_support_1d = {
    {["not_support"] = 0} -- 一当前不支持
}
local not_support_1e = {
    {["not_support"] = 0} -- 一当前不支持
}
local not_support_1f = {
    {["not_support"] = 0} -- 一当前不支持
}
-- oodc 0102属性
local remote_query = {
    [error_num_0] = {0X00, 0X01}, -- 室内目标频率
    [T1_temperature_01] = {0X01, 0X02}, -- t1
    [T2_temperature_02] = {0X02, 0X02}, ---t2 温度
    [T3_temperature_03] = {0X03, 0X02}, -- t3
    [T4_temperature_04] = {0X04, 0X02}, -- t4
    [T5_temperature_05] = {0X05, 0X02}, -- t5
    [compressor_target_frequency_06] = {0X06, 0X01}, -- 压缩机目标频率
    [compressor_actual_frequency_07] = {0X07, 0X01}, -- 压缩机目标频率
    [ac_parameter_08] = {0X08, 0X02}, -- 空调参数
    [indoor_machine_parameter_09] = {0X09, 0X01}, -- 室内机参数
    [indoor_machine_parameter_0a] = {0X0a, 0x01}, -- 室内目标频率
    [indoor_running_mode_0b] = {0X0b, 0x01}, -- 室内目标频率
    [outdoor_fan_setting_speed_0c] = {0x0c, 0x02}, -- 室外风机设定转速
    [electronic_expansion_open_0d] = {0x0d, 0x02}, -- 电子膨胀阀开度
    [outdoor_fan_actual_speed_0e] = {0x0e, 0x02}, -- 室外风机设定转速
    [indoor_humidity_0f] = {0x0f, 0x01}, -- 室内湿度
    [compensate_temperature_10] = {0x10, 0x02}, -- 补偿温度
    [not_support_11] = {0x11, 0x00},
    [wifi_singal_strength_12] = {0x12, 0x01}, -- wifi信号强度
    [DC_bus_voltage_13] = {0x13, 0x01}, -- wifi信号强度
    [indoor_target_frequency_14] = {0x14, 0x01}, -- 室内目标频率
    [T2A_temperature_15] = {0x15, 0x01}, -- 室内目标频率
    [T2B_temperature_16] = {0x16, 0x01}, -- 室内目标频率
    [not_support_17] = {0x17, 0x00},
    [not_support_18] = {0x18, 0x00},
    [not_support_19] = {0x19, 0x00},
    [not_support_1a] = {0x1a, 0x00},
    [not_support_1b] = {0x1b, 0x00},
    [not_support_1c] = {0x1c, 0x00},
    [not_support_1d] = {0x1d, 0x00},
    [not_support_1e] = {0x1e, 0x00},
    [not_support_1f] = {0x1f, 0x00}
}

-- 00dd 0103属性
-- 通道0 能力匹数设置
local ability_match_setting_00 = {
    {["query_current_ability"] = 1} -- 能力匹数设置
}
-- 掉电记忆选择
local power_down_memory_01 = {
    {["power_down_memory_choose"] = 1} -- 掉电记忆选择
}
-- 达温后的内风机控制
local arrive_temp_internal_fan_02 = {
    {["internal_fan_status"] = 1} -- 达温后的内风机控制
}
-- 模式选择
local mode_choose_03 = {
    {["mode_choose"] = 1} -- 模式选择
}
-- 最低设定温度选择（可调范围16~24度）
local low_setting_temp_04 = {
    {["low_setting_temp"] = 1} -- 最低设定温度选择（可调范围16~24度）
}
-- 最高设定温度选择（可调范围25~30度）
local high_setting_temp_05 = {
    {["high_setting_temp"] = 1} -- 最高设定温度选择（可调范围25~30度）
}
-- 一拖多制冷制热优先选择
local one_many_priority_select_06 = {
    {["one_many_priority_select"] = 1} -- 一拖多制冷制热优先选择
}

local not_support_07 = {
    {["not_support"] = 0} -- 一当前不支持
}

local not_support_08 = {
    {["not_support"] = 0} -- 一当前不支持
}

local not_support_09 = {
    {["not_support"] = 0} -- 一当前不支持
}

local not_support_0a = {
    {["not_support"] = 0} -- 一当前不支持
}
-- 制冷最小频率选择（10~50Hz）
local refrigeration_min_frequency_0b = {
    {["refrigeration_min_frequency"] = 1} -- 制冷最小频率选择（10~50Hz）
}
-- 制热最小频率选择（10~50Hz）
local heat_min_frequency_0c = {
    {["heat_min_frequency"] = 1} -- 制热最小频率选择（10~50Hz）
}
-- 制冷模式T4限制区间6最大运行频率选择（20~150Hz）
local cool_t4_rang_max_freq_0d = {
    {["cool_t4_rang_max_freq"] = 1} -- 制冷模式T4限制区间6最大运行频率选择（20~150Hz）
}
-- 选择共振点和幅值
local choose_resonance_point_value_0e = {
    {["choose_resonance_point"] = 1}, -- 选择共振点和幅值
    {["choose_value"] = 1} -- 选择共振点和幅值
}
-- 室外强制运行频率设定
local outdoor_force_running_frequence_0f = {
    {["outdoor_force_running_frequence"] = 1} -- 室外强制运行频率设定
}

local one_click_factory_reset_10 = {
    {["one_click_factory_reset"] = 1} -- 一当前不支持
}
local not_support_11 = {
    {["not_support"] = 0} -- 一当前不支持
}
-- 机型选择
local machine_type_select_12 = {
    {["machine_type_select"] = 1} -- 机型选择 23~100
}
-- 制冷最大频率设定
local cool_max_frequence_13 = {
    {["cool_max_frequence"] = 1} -- 制冷最大频率设定
}

-- 制热最大频率设定
local heat_max_frequence_14 = {
    {["heat_max_frequence"] = 1} -- 制热最大频率设定
}

-- 制冷设定温度补偿值设置
local refrigeration_temp_compensation_value_15 = {
    {["refrigeration_temp_compensation_value"] = 1} -- 制冷设定温度补偿值设置
}

-- 制热设定温度补偿值设置
local heat_temp_compensation_value_16 = {
    {["heat_temp_compensation_value"] = 1} -- 制热设定温度补偿值设置
}

-- 制冷最大风速设置
local refrigeration_max_wind_speed_setting_17 = {
    {["refrigeration_max_wind_speed_setting"] = 1} -- 制热设定温度补偿值设置
}

-- 制冷最小风速设置
local refrigeration_mix_wind_speed_setting_18 = {
    {["refrigeration_min_wind_speed_setting"] = 1} -- 制热设定温度补偿值设置
}

-- 制热最大风速设置
local heat_max_wind_speed_setting_19 = {
    {["heat_max_wind_speed_setting"] = 1} -- 制热最大风速设置
}

-- 制热最小风速设置
local heat_mix_wind_speed_setting_1a = {
    {["heat_min_wind_speed_setting"] = 1} -- 制热最小风速设置
}
local not_support_1b = {
    {["not_support"] = 0} -- 一当前不支持
}
...
local remote_control = {
    [ability_match_setting_00] = {0x00, 0x01, 0x35},
    [power_down_memory_01] = {0x01, 0x01, 0x15},
    [arrive_temp_internal_fan_02] = {0x02, 0x01, 0x0b},
    [mode_choose_03] = {0x03, 0x01, 0x0d},
    [low_setting_temp_04] = {0x04, 0x01, 0x13},
    [high_setting_temp_05] = {0x05, 0x01, 0x14},
    [one_many_priority_select_06] = {0x06, 0x01, 0x24},
    [not_support_07] = {0x07, 0x00, 0xff},
    [not_support_08] = {0x08, 0x00, 0xff},
    [not_support_09] = {0x09, 0x00, 0xff},
    [not_support_0a] = {0x0a, 0x00, 0xff},
    [refrigeration_min_frequency_0b] = {0x0b, 0x01, 0x1a},
    [heat_min_frequency_0c] = {0x0c, 0x01, 0x1b},
    [cool_t4_rang_max_freq_0d] = {0x0d, 0x01, 0x1c},
    [choose_resonance_point_value_0e] = {0x0e, 0x02, 0x1d},
    [outdoor_force_running_frequence_0f] = {0x0f, 0x01, 0x36},
    [one_click_factory_reset_10] = {0x10, 0x01, 0xff},
    [not_support_11] = {0x11, 0x01, 0xff},
    [machine_type_select_12] = {0x12, 0x01, 0x00},
    [cool_max_frequence_13] = {0x13, 0x01, 0x01},
    [heat_max_frequence_14] = {0x14, 0x01, 0x02},
    [refrigeration_temp_compensation_value_15] = {0x15, 0x01, 0x04},
    [heat_temp_compensation_value_16] = {0x16, 0x01, 0x05},
    [refrigeration_max_wind_speed_setting_17] = {0x17, 0x01, 0x06},
    [refrigeration_mix_wind_speed_setting_18] = {0x18, 0x01, 0x07},
    [heat_max_wind_speed_setting_19] = {0x19, 0x01, 0x08},
    [heat_mix_wind_speed_setting_1a] = {0x1a, 0x01, 0x09},
    [not_support_1b] = {0x1b, 0x01, 0xff},
    [not_support_1c] = {0x1c, 0x01, 0xff},
    [not_support_1d] = {0x1d, 0x01, 0xff},
    [not_support_1e] = {0x1e, 0x01, 0xff},
    [not_support_1f] = {0x1e, 0x01, 0xff}
}
...
local function init_keyP()
    ...
    keyP["continuous_wind"] = nil
    keyP["b5_continuous_wind"] = nil
    keyP["async_wind_swing_lr"] = nil
    keyP["b5_async_wind_swing_lr"] = nil
    keyP["b5_request_time"] = nil
    keyP["current_time_year"] = nil
    keyP["current_time_month"] = nil
    keyP["current_time_week"] = nil
    keyP["current_time_day"] = nil
    keyP["current_time_hour"] = nil
    keyP["current_time_min"] = nil
    keyP["current_time_second"] = nil

    keyP["operating_frequency_max"] = nil
    keyP["operating_frequency_min"] = nil
    keyP["inner_operating_max"] = nil
    keyP["inner_operating_min"] = nil
    keyP["heat_operating_frequency_max"] = nil
    keyP["heat_operating_frequency_min"] = nil
    keyP["heat_inner_operating_max"] = nil
    keyP["heat_inner_operating_min"] = nil
    keyP["frequency_point_1"] = nil
    keyP["frequency_point_2"] = nil
    keyP["frequency_point_3"] = nil
    keyP["frequency_point_4"] = nil
    keyP["frequency_point_5"] = nil
    keyP["frequency_point_amplitude_1"] = nil
    keyP["frequency_point_amplitude_2"] = nil
    keyP["frequency_point_amplitude_3"] = nil
    keyP["frequency_point_amplitude_4"] = nil
    keyP["frequency_point_amplitude_5"] = nil
    keyP["three_dimensional_power_saving"] = nil
    keyP["three_dimensional_power_saving_time"] = nil
    keyP["b5_three_dimensional_power_saving"] = nil
    keyP["power_saving_screen"] = nil
    keyP["b5_power_saving_screen"] = nil
    keyP["dynamic_temp_compensation"] = nil
    keyP["frequency_compensation"] = nil
    keyP["meeting_mode"] = nil
    keyP["control_humidity"] = nil
    keyP["control_humidity_level"] = nil
    keyP["b5_cool_heat"] = nil
    keyP["prevent_straight_wind_speed"] = nil
    keyP["b5_prevent_straight_wind_speed"] = nil
    keyP["comfort_dry_value"] = nil
    keyP["mute"] = nil
    keyP["one_control"] = nil
    keyP["one_control_code"] = nil
    keyP["b5_mute"] = nil
    keyP["b5_one_control"] = nil
    keyP["follow_body_sense"] = nil
    keyP["fan_mode_speed_support"] = nil
    keyP["b5_fan_mode_speed_support"] = nil
    -- 一拖多外机实际功率
    keyP["outdoor_real_power"] = nil
    -- 一拖多外机系统总功率
    keyP["outdoor_system_total_power"] = nil
    keyP["b5_remote_query"] = nil
    keyP["b5_remote_control"] = nil
    keyP["cool_power_saving"] = nil
    keyP["b5_cool_power_saving"] = nil
    keyP["degerming"] = nil
    keyP["b5_degerming"] = nil
    keyP["b5_prevent_super_cool"] = nil
    keyP["insta_cool"] = nil
    keyP["b5_insta_cool"] = nil
    keyP["real_time_power_display"] = nil
    keyP["b5_real_time_power_display"] = nil
    keyP["b5_gentle_wind_sense"] = nil
    keyP["fresh_air_mode"] = nil
    keyP["light"] = nil
    keyP["b5_light"] = nil
    keyP["imax_wind"] = nil
    keyP["b5_imax_wind"] = nil
    keyP["quick_cool_heat"] = nil
    keyP["b5_quick_cool_heat"] = nil
    keyP["prevent_super_cool_2"] = nil
    keyP["has_prevent_super_cool_2"] = nil
    keyP["b5_prevent_super_cool_2"] = nil
    for key, value in pairs(remote_query) do
        for i, j in pairs(key) do
            for k, v in pairs(j) do keyP[k] = nil end
        end
    end
    for key, value in pairs(remote_control) do
        for i, j in pairs(key) do
            for k, v in pairs(j) do keyP[k] = nil end
        end
    end
    if keyP["unchange_value_channel_0e"] ~= nil then
        for i = 1, keyP["unchange_value_channel_0e"] do
            keyP["channel_" .. i] = nil
            keyP["channel_value_" .. i] = nil
        end
        keyP["unchange_value_channel_0e"] = nil
    end
end
...
local function convert_to_F(temp)
    ...
    if (type(temp) == "string") then temperature = tonumber(temp) end
...
local function jsonToModel(jsonCmd, jsonType)
    ...
    if (streams[keyT["KEY_SMART_DRY"]] ~= nil) then
        ...
        if (jsonType == "control") then
            keyP["comfort_dry_value"] = keyP["smartDryValue"]
        end
    end
    -- comfort除湿值
    if (streams["comfort_dry_value"] ~= nil) then
        keyP["comfort_dry_value"] = checkBoundary(streams["comfort_dry_value"],
                                                  30, 101)
        if (jsonType == "control") then
            keyP["smartDryValue"] = keyP["comfort_dry_value"]
        end
    end
    -- 自然风
    ...
    if (streams[keyT["KEY_TEMPERATURE"]] ~= nil) then
        if (type(streams[keyT["KEY_TEMPERATURE"]]) == "string") then
            streams[keyT["KEY_TEMPERATURE"]] = tonumber(
                                                   streams[keyT["KEY_TEMPERATURE"]])
        end
        if (streams[keyT["KEY_TEMPERATURE"]] < 40) then
            keyP["temperature"] = checkBoundary(
                                      streams[keyT["KEY_TEMPERATURE"]], 8, 30)
        else
            keyP["temperature"] = checkBoundary(
                                      streams[keyT["KEY_TEMPERATURE"]], 46, 86)
    ...
    if (streams["temperature_unit"] ~= nil) then
        keyP["temperature_unit"] = checkBoundary(streams["temperature_unit"], 0,
                                                 1)
    end
    ...
    if (streams[keyT["KEY_DEGREE8_HEAT"]] ~= nil) then
        keyP["degree8_heat"] = checkBoundary(streams[keyT["KEY_DEGREE8_HEAT"]],
                                             0, 1)
    end
    ...
    if (jsonType == "control" and streams[keyT["KEY_FA_NO_WIND_SENSE"]] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["fa_no_wind_sense"] = checkBoundary(
                                       streams[keyT["KEY_FA_NO_WIND_SENSE"]], 1,
                                       4)
    end
    ...
    if (streams["sup_horizontal_guide_strip_1"] ~= nil) then
        keyP["sup_horizontal_guide_strip_1"] =
            streams["sup_horizontal_guide_strip_1"]
    end
    if (streams["sup_horizontal_guide_strip_2"] ~= nil) then
        keyP["sup_horizontal_guide_strip_2"] =
            streams["sup_horizontal_guide_strip_2"]
    end
    if (streams["sup_horizontal_guide_strip_3"] ~= nil) then
        keyP["sup_horizontal_guide_strip_3"] =
            streams["sup_horizontal_guide_strip_3"]
    end
    if (streams["sup_horizontal_guide_strip_4"] ~= nil) then
        keyP["sup_horizontal_guide_strip_4"] =
            streams["sup_horizontal_guide_strip_4"]
    end
    if (jsonType == "control" and
        (streams["sup_horizontal_guide_strip_1"] ~= nil or
            streams["sup_horizontal_guide_strip_2"] ~= nil or
            streams["sup_horizontal_guide_strip_3"] ~= nil or
            streams["sup_horizontal_guide_strip_4"] ~= nil)) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
    ...
    if (streams["prevent_straight_wind_flag"] ~= nil) then
        keyP["prevent_straight_wind_flag"] =
            streams["prevent_straight_wind_flag"]
    end
    ...
    -- 随身感
    if (streams["follow_body_sense"] ~= nil) then
        if (streams["follow_body_sense"] == 1) then
            keyP["follow_body_sense"] = 0x80
        else
            keyP["follow_body_sense"] = streams["follow_body_sense"]
        end
    end
    ...
    if (jsonType == "control" and streams["prevent_straight_wind_select"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["prevent_straight_wind_select"] =
            streams["prevent_straight_wind_select"]
    end
    -- CVP
    ...
    -- 连续风
    if (jsonType == "control" and streams["continuous_wind"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["continuous_wind"] = streams["continuous_wind"]
    end
    -- 单面风
    if (jsonType == "control" and streams["async_wind_swing_lr"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["async_wind_swing_lr"] = streams["async_wind_swing_lr"]
    end

    -- 当地网络时间
    if (jsonType == "control" and streams["current_time_year"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["current_time_year"] = streams["current_time_year"]
    end
    if (streams["current_time_month"] ~= nil) then
        keyP["current_time_month"] = streams["current_time_month"]
    end
    if (streams["current_time_week"] ~= nil) then
        keyP["current_time_week"] = streams["current_time_week"]
    end
    if (streams["current_time_day"] ~= nil) then
        keyP["current_time_day"] = streams["current_time_day"]
    end
    if (streams["current_time_hour"] ~= nil) then
        keyP["current_time_hour"] = streams["current_time_hour"]
    end
    if (streams["current_time_min"] ~= nil) then
        keyP["current_time_min"] = streams["current_time_min"]
    end
    if (streams["current_time_second"] ~= nil) then
        keyP["current_time_second"] = streams["current_time_second"]
    end

    -- 3D睡眠
    if (jsonType == "control" and streams["three_dimensional_power_saving"] ~=
        nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["three_dimensional_power_saving"] =
            streams["three_dimensional_power_saving"]
        keyP["three_dimensional_power_saving_time"] = 4
    end
    if (streams["three_dimensional_power_saving_time"] ~= nil) then
        keyP["three_dimensional_power_saving_time"] =
            streams["three_dimensional_power_saving_time"]
    end

    -- 3D睡眠屏显
    if (jsonType == "control" and streams["power_saving_screen"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["power_saving_screen"] = streams["power_saving_screen"]
    end
    ...
    -- 会客模式
    if (jsonType == "control" and streams["meeting_mode"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["meeting_mode"] = streams["meeting_mode"]
    end

    -- 控温
    if (jsonType == "control" and streams["control_humidity"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["control_humidity"] = streams["control_humidity"]
    end
    if (streams["control_humidity_level"] ~= nil) then
        keyP["control_humidity_level"] = streams["control_humidity_level"]
    end
    -- 防直吹默认风速
    if (jsonType == "control" and streams["prevent_straight_wind_speed"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["prevent_straight_wind_speed"] =
            streams["prevent_straight_wind_speed"]
    end

    -- 静音
    if (jsonType == "control" and streams["mute"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["mute"] = streams["mute"]
    end
    -- 酷省电
    ...
    -- insta cool
    if (jsonType == "control" and streams["insta_cool"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["insta_cool"] = streams["insta_cool"]
    end
    -- 显示板实时功率显示
    if (jsonType == "control" and streams["real_time_power_display"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["real_time_power_display"] = streams["real_time_power_display"]
    end
    -- 远程控制模式
    if (jsonType == "control" and streams["control_channel"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
        keyP["control_channel"] = streams["control_channel"]
        for i, j in pairs(remote_control) do
            for k, v in pairs(i) do
                for key, value in pairs(v) do
                    if (streams[key] ~= nil) then
                        keyP[key] = streams[key]
                    end
                end
            end
        end
    ...
    -- 智控温2.0
    if (jsonType == "control" and streams["prevent_super_cool_2"] ~= nil) then
        keyP["propertyNumber"] = keyP["propertyNumber"] + 1
...
local function binToModel(binData, deviceSN8)
    ...
    if ((dataType == 0x02 and messageBytes[0] == 0xC0) or
        ...
        if (dataType == 0x05) then
            keyP["comfort_dry_value"] = bit.band(messageBytes[13], 0x7F)
        else
            keyP["comfort_dry_value"] = bit.band(messageBytes[19], 0x7F)
        end
        ...
        if (dataType == 0x05) then
            ...
            if deviceSN8 == "11447" or deviceSN8 == "11451" or deviceSN8 ==
                ...
                local temp = bit.rshift(bit.band(messageBytes[1], 0x3E), 1)
        ...
        keyP["follow_body_sense"] = bit.rshift(bit.band(messageBytes[8], 0x80),
                                               7)
        ...
        keyP["fresh_filter_timeout_ae2"] = bit.rshift(
                                               bit.band(messageBytes[13], 0x20),
                                               5)
        ...
        if (dataType == 0x02 or dataType == 0x03) then
            ...
            keyP["smallOutdoorTemperatureValue"] =
                bit.rshift(messageBytes[15], 4);
            keyP["fresh_filter_timeout"] = bit.rshift(
                                               bit.band(messageBytes[13], 0x40),
                                               6)
            -- end
        end
        if (dataType ~= 0x05) then keyP["errorCode"] = messageBytes[16] end

        -- 温度单位
        if (dataType == 0x05) then
            keyP["temperature_unit"] = bit.rshift(
                                           bit.band(messageBytes[9], 0x80), 7)
        else
            keyP["temperature_unit"] = bit.rshift(
        ...
        -- 屏显状态
        keyP["screenDisplayNowValue"] = bit.rshift(
                                            bit.band(messageBytes[14], 0x70), 4)
        -- 舒省
        ...
        if (#binData >= 23) then
            -- 8度制热
            if (messageBytes[0] == 0xA0) then
                keyP["degree8_heat"] = bit.rshift(
                                           bit.band(messageBytes[12], 0x80), 7)
            else
                keyP["degree8_heat"] = bit.rshift(
                                           bit.band(messageBytes[21], 0x80), 7)
            end
        end

        -- 单独电辅热
        if (dataType == 0x05) then
            keyP["independent_ptc"] = bit.rshift(
                                          bit.band(messageBytes[11], 0x08), 3)
        else
            keyP["independent_ptc"] = bit.rshift(
                                          bit.band(messageBytes[8], 0x40), 6)
        end
    ...
    if ((dataType == 0x02 and messageBytes[0] == 0xB0) or
        ...
        for i = 1, keyP["propertyNumber"] do
            ...
            if (messageBytes[cursor + 0] == 0xE0 and messageBytes[cursor + 1] ==
                ...
                keyP["ieco_target_rate"] =
                    bit.bor(messageBytes[cursor + 5],
                            bit.bor(bit.lshift(messageBytes[cursor + 6], 8)))
                keyP["ieco_indoor_wind_speed_level"] = messageBytes[cursor + 7]
                keyP["ieco_indoor_wind_speed"] = bit.bor(
                ...
                keyP["ieco_outdoor_wind_speed"] = bit.bor(
            ...
            if (messageBytes[cursor + 0] == 0x8D and messageBytes[cursor + 1] ==
                0x00) then
                keyP["mito_cool"] = (messageBytes[cursor + 4] - 50) / 2
                cursor = cursor + 5
            end
            if (messageBytes[cursor + 0] == 0x8E and messageBytes[cursor + 1] ==
                0x00) then
                keyP["mito_heat"] = (messageBytes[cursor + 4] - 50) / 2
                cursor = cursor + 5
            end
            if (messageBytes[cursor + 0] == 0x8F and messageBytes[cursor + 1] ==
                0x00) then
                keyP["dr_time_min"] = messageBytes[cursor + 4]
                keyP["dr_time_hour"] = messageBytes[cursor + 5]
                cursor = cursor + 6
            end
            if (messageBytes[cursor + 0] == 0x90 and messageBytes[cursor + 1] ==
                0x00) then
                ...
                keyP["cool_strong_wind_speed"] = bit.bor(
                                                     messageBytes[cursor + 11],
                                                     bit.bor(
                                                         bit.lshift(
                                                             messageBytes[cursor +
                                                                 12], 8)))
                keyP["cool_strong_wind_amount"] = bit.bor(
            ...
            if (messageBytes[cursor + 0] == 0x91 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["has_icheck"] = messageBytes[cursor + 4]
                cursor = cursor + 5
            end
            if (messageBytes[cursor + 0] == 0x98 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["cvp"] = messageBytes[cursor + 4]
                cursor = cursor + 5
            end
            if (messageBytes[cursor + 0] == 0xAA and messageBytes[cursor + 1] ==
                0x00) then
                keyP["new_wind_sense"] = messageBytes[cursor + 4]
                cursor = cursor + 5
            end
            if (messageBytes[cursor + 0] == 0xAD and messageBytes[cursor + 1] ==
                0x00) then
                keyP["comfort"] = messageBytes[cursor + 4]
                cursor = cursor + 5
            end
            if (messageBytes[cursor + 0] == 0x8C and messageBytes[cursor + 1] ==
                0x00) then
                keyP["continuous_wind"] = messageBytes[cursor + 4]
                cursor = cursor + 5
            end
            if (messageBytes[cursor + 0] == 0xBD and messageBytes[cursor + 1] ==
                0x00) then
                keyP["async_wind_swing_lr"] = messageBytes[cursor + 4]
                cursor = cursor + 5
            end
            ...
            if (messageBytes[cursor + 0] == 0xC0 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["power_saving_screen"] = messageBytes[cursor + 4]
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            if (messageBytes[cursor + 0] == 0xC1 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["meeting_mode"] = messageBytes[cursor + 4]
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            if (messageBytes[cursor + 0] == 0xCB and messageBytes[cursor + 1] ==
                0x00) then
                keyP["prevent_straight_wind_speed"] = messageBytes[cursor + 4]
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            if (messageBytes[cursor + 0] == 0x7C and messageBytes[cursor + 1] ==
                0x00) then
                keyP["control_humidity"] = messageBytes[cursor + 4]
                keyP["control_humidity_level"] = messageBytes[cursor + 5]
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            if (messageBytes[cursor + 0] == 0xB4 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["operating_frequency_max"] = messageBytes[cursor + 4]
                keyP["operating_frequency_min"] = messageBytes[cursor + 5]
                keyP["inner_operating_max"] = messageBytes[cursor + 6] * 8
                keyP["inner_operating_min"] = messageBytes[cursor + 7] * 8
                keyP["frequency_point_1"] = messageBytes[cursor + 8]
                keyP["frequency_point_2"] = messageBytes[cursor + 9]
                keyP["frequency_point_3"] = messageBytes[cursor + 10]
                keyP["frequency_point_4"] = messageBytes[cursor + 11]
                keyP["frequency_point_5"] = messageBytes[cursor + 12]
                keyP["frequency_point_amplitude_1"] = messageBytes[cursor + 13]
                keyP["frequency_point_amplitude_2"] = messageBytes[cursor + 14]
                keyP["frequency_point_amplitude_3"] = messageBytes[cursor + 15]
                keyP["frequency_point_amplitude_4"] = messageBytes[cursor + 16]
                keyP["frequency_point_amplitude_5"] = messageBytes[cursor + 17]
                keyP["heat_operating_frequency_max"] = messageBytes[cursor + 18]
                keyP["heat_operating_frequency_min"] = messageBytes[cursor + 19]
                keyP["heat_inner_operating_max"] = messageBytes[cursor + 20] * 8
                keyP["heat_inner_operating_min"] = messageBytes[cursor + 21] * 8
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            if (messageBytes[cursor + 0] == 0xAB and messageBytes[cursor + 1] ==
                0x00) then
            ...
            if (messageBytes[cursor + 0] == 0xAC and messageBytes[cursor + 1] ==
                0x00) then
                if (messageBytes[cursor + 2] == 0x11 or messageBytes[cursor + 2] ==
                    0x10 or messageBytes[cursor + 3] == 0x00) then
                    cursor = cursor + 4
            ...
            if (messageBytes[cursor + 0] == 0xC2 and messageBytes[cursor + 1] ==
                0x00) then
                if (messageBytes[cursor + 2] == 0x11 or messageBytes[cursor + 2] ==
                    0x10 or messageBytes[cursor + 3] == 0x00) then
                    cursor = cursor + 4
                else
                    keyP["one_control"] = messageBytes[cursor + 4]

                    local outCodeString = ""

                    for i = 0, 5 do
                        local stringChar =
                            string.format("%02x", messageBytes[cursor + i + 5])
                        outCodeString = outCodeString .. stringChar
                    end

                    keyP["one_control_code"] = outCodeString
                    cursor = cursor + messageBytes[cursor + 3] + 4
                end
            end
            if (messageBytes[cursor + 0] == 0xCD and messageBytes[cursor + 1] ==
                0x00) then
                keyP["mute"] = messageBytes[cursor + 4]
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            if (messageBytes[cursor + 0] == 0xCE and messageBytes[cursor + 1] ==
                0x00) then
                keyP["fan_mode_speed_support"] = messageBytes[cursor + 4]
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            ...
            if (messageBytes[cursor + 0] == 0x00 and messageBytes[cursor + 1] ==
                0x01) then
                keyP["insta_cool"] = messageBytes[cursor + 4]
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            if (messageBytes[cursor + 0] == 0x01 and messageBytes[cursor + 1] ==
                0x01) then
                keyP["real_time_power_display"] = messageBytes[cursor + 4]
                cursor = cursor + messageBytes[cursor + 3] + 4
            end
            ...
            if (messageBytes[cursor + 0] == 0xD2 and messageBytes[cursor + 1] ==
                ...
                if (messageBytes[cursor + 2] == 0x11 or messageBytes[cursor + 2] ==
                    ...
                    -- keyP["prevent_super_cool_mode_select"] = messageBytes[cursor + 5]
                    cursor = cursor + messageBytes[cursor + 3] + 4
                end
            ...
            if (messageBytes[cursor + 0] == 0x02 and messageBytes[cursor + 1] ==
                0x01) then
                local remoteQueryLength = messageBytes[cursor + 3]
                cursor = cursor + 3
                local queryLength = 0
                for num = 1, getTableLength(remote_query) do
                    if queryLength < remoteQueryLength then
                        for i, j in pairs(remote_query) do
                            if (messageBytes[cursor + 1] == j[1]) then
                                remote_query_info(i, messageBytes, cursor, 3,
                                                  messageBytes[cursor + 2])
                                queryLength = queryLength +
                                                  messageBytes[cursor + 2] + 2
                                cursor = cursor + messageBytes[cursor + 2] + 2
                                break
                            end
                        end
                    else
            ...
            if (messageBytes[cursor + 0] == 0x03 and messageBytes[cursor + 1] ==
                0x01) then
                local remoteControlLength = messageBytes[cursor + 3]
                cursor = cursor + 3
                local ControlLength = 0
                for num = 1, getTableLength(remote_control) do
                    if ControlLength < remoteControlLength then
                        for i, j in pairs(remote_control) do
                            if (messageBytes[cursor + 1] == j[1] and
                                messageBytes[cursor + 1] ~= 0x0e) then
                                remote_control_info(i, messageBytes, cursor, 3,
                                                    messageBytes[cursor + 2])
                                ControlLength = ControlLength +
                                                    messageBytes[cursor + 2] + 2
                                cursor = cursor + messageBytes[cursor + 2] + 2
                                break
                            elseif (messageBytes[cursor + 1] == j[1] and
                                messageBytes[cursor + 1] == 0x0e) then
                                ControlLength = ControlLength +
                                                    messageBytes[cursor + 2] + 2
                                cursor =
                                    remote_control_info_channel_0e(i,
                                                                   messageBytes,
                                                                   cursor, 3,
                                                                   messageBytes[cursor +
                                                                       2])
            ...
            -- 控制返回
            if (messageBytes[cursor + 0] == 0xB1 and messageBytes[cursor + 1] ==
                0x00) then
                for i, j in pairs(remote_control) do
                    if (messageBytes[cursor + 4] == j[3]) then
                        remote_control_result(i, messageBytes, cursor, 4)
                    end
                end
                cursor = cursor + 3
            end
        ...
        if (#messageBytes > 7) then
            if (messageBytes[3] == 0x44) then
                ...
                if (messageBytes[24] ~= nil) then
                    keyP["outdoor_real_power"] = bit.bor(
                                                     bit.bor(bit.lshift(
                                                                 messageBytes[19],
                                                                 16),
                                                             bit.lshift(
                                                                 ...
                                                                 8)),
                                                     messageBytes[21])
                    keyP["outdoor_system_total_power"] = bit.bor(bit.bor(
                                                                     bit.lshift(
                                                                         messageBytes[22],
                                                                         16),
                                                                     bit.lshift(
    ...
    if ((dataType == 0x05 and messageBytes[0] == 0xB5)) then
        ...
        local cursor = 1
        for i = 1, keyP["propertyNumber"] do
            if (messageBytes[cursor + 1] == 0x7E and messageBytes[cursor + 2] ==
                0x00) then
                keyP["propertyNumber"] = 0
                ...
                if (dataType == 0x05) then
                    ...
                    if deviceSN8 == "11447" or deviceSN8 == "11451" or deviceSN8 ==
                        ...
                        local temp = bit.rshift(bit.band(
                                                    messageBytes[cursor + 5],
                                                    0x3E), 1)
                        if (temp > 0 and temp <= 25) then
                            keyP["temperature"] = temp + 12
                ...
                -- keyP["follow_body_sense"] = bit.rshift(bit.band(messageBytes[cursor + 12], 0x80),7)
                ...
                keyP["temperature_unit"] =
                    bit.rshift(bit.band(messageBytes[cursor + 13], 0x80), 7)
                ...
                keyP["ieco_switch"] = bit.rshift(bit.band(
                                                     messageBytes[cursor + 45],
                                                     0x10), 4)
                keyP["linkage_fan_speed"] =
                    bit.band(messageBytes[cursor + 42], 0x7F)
    ...
    if ((dataType == 0x03 and messageBytes[0] == 0xB5)) then
        -- 新协议，变长属性协议
        if (#binData < 4) then return nil end
        keyP["propertyNumber"] = messageBytes[1]
        local cursor = 2
        for i = 1, keyP["propertyNumber"] do
            ...
            if (messageBytes[cursor + 0] == 0x1f and messageBytes[cursor + 1] ==
                0x02) then
            ...
            if (messageBytes[cursor + 0] == 0x12 and messageBytes[cursor + 1] ==
                0x02) then
            ...
            if (messageBytes[cursor + 0] == 0x13 and messageBytes[cursor + 1] ==
                0x02) then
                keyP["b5_8_heat"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0x16 and messageBytes[cursor + 1] ==
                0x02) then
                keyP["b5_electricity"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0x19 and messageBytes[cursor + 1] ==
                0x02) then
            ...
            if (messageBytes[cursor + 0] == 0x30 and messageBytes[cursor + 1] ==
                0x02) then
                keyP["b5_main_horizontal_guide_strip_1"] =
                    messageBytes[cursor + 3]
                keyP["b5_main_horizontal_guide_strip_2"] =
                    messageBytes[cursor + 4]
                keyP["b5_main_horizontal_guide_strip_3"] =
                    messageBytes[cursor + 5]
                keyP["b5_main_horizontal_guide_strip_4"] =
                    messageBytes[cursor + 6]
                cursor = cursor + 7
            end
            if (messageBytes[cursor + 0] == 0x31 and messageBytes[cursor + 1] ==
                0x02) then
                keyP["b5_sup_horizontal_guide_strip_1"] =
                    messageBytes[cursor + 3]
                keyP["b5_sup_horizontal_guide_strip_2"] =
                    messageBytes[cursor + 4]
                keyP["b5_sup_horizontal_guide_strip_3"] =
                    messageBytes[cursor + 5]
                keyP["b5_sup_horizontal_guide_strip_4"] =
                    messageBytes[cursor + 6]
                cursor = cursor + 7
            end
            if (messageBytes[cursor + 0] == 0x32 and messageBytes[cursor + 1] ==
            ...
            if (messageBytes[cursor + 0] == 0x58 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_prevent_straight_wind_select"] =
                    messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            ...
            if (messageBytes[cursor + 0] == 0x93 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_emergent_heat_wind"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0x94 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_heat_ptc_wind"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0x95 and messageBytes[cursor + 1] ==
            ...
            if (messageBytes[cursor + 0] == 0x6D and messageBytes[cursor + 1] ==
                0x00) then cursor = cursor + 4 end
            if (messageBytes[cursor + 0] == 0xAD and messageBytes[cursor + 1] ==
                0x00) then
            ...
            if (messageBytes[cursor + 0] == 0x8C and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_continuous_wind"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0x4D and messageBytes[cursor + 1] ==
                0x00) then cursor = cursor + 5 end
            if (messageBytes[cursor + 0] == 0x1A and messageBytes[cursor + 1] ==
                0x00) then cursor = cursor + 4 end
            if (messageBytes[cursor + 0] == 0xBD and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_async_wind_swing_lr"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0xBE and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_request_time"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0xB4 and messageBytes[cursor + 1] ==
                0x00) then
            ...
            if (messageBytes[cursor + 0] == 0x11 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_three_dimensional_power_saving"] =
                    messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xC0 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_power_saving_screen"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xC1 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_meeting_mode"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0x7C and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_control_humidity"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0x9C and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_cool_heat"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0x90 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_cool_heat_amount"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xCB and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_prevent_straight_wind_speed"] =
                    messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xCD and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_mute"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xCE and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_fan_mode_speed_support"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xC2 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_one_control"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end

            if (messageBytes[cursor + 0] == 0xb0 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_remote_query"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end

            if (messageBytes[cursor + 0] == 0xb1 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_remote_control"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0xE3 and messageBytes[cursor + 1] ==
                0x00) then
            ...
            if (messageBytes[cursor + 0] == 0x89 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_cool_power_saving"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0x5A and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_degerming"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0x49 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_prevent_super_cool"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0x00 and messageBytes[cursor + 1] ==
                0x01) then
                keyP["b5_insta_cool"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0x01 and messageBytes[cursor + 1] ==
                0x01) then
                keyP["b5_real_time_power_display"] = messageBytes[cursor + 3]
                cursor = cursor + messageBytes[cursor + 2] + 3
            end
            if (messageBytes[cursor + 0] == 0x9D and messageBytes[cursor + 1] ==
                0x00) then cursor = cursor + 4 end
            if (messageBytes[cursor + 0] == 0x86 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_gentle_wind_sense"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0x5B and messageBytes[cursor + 1] ==
                0x00) then
            ...
            if (messageBytes[cursor + 0] == 0xB3 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_quick_cool_heat"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0xD3 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_imax_wind"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
            if (messageBytes[cursor + 0] == 0xD2 and messageBytes[cursor + 1] ==
                0x00) then
                keyP["b5_prevent_super_cool_2"] = messageBytes[cursor + 3]
                cursor = cursor + 4
            end
        end
    end
...
function jsonToData(jsonCmd)
    ...
    if (query) then
        ...
        elseif (queryType == "power" or queryType == "purifier" or queryType ==
            ...
            "tubro" or queryType == "current_work_time" or queryType ==
            "comfort_power_save" or queryType == "comfort_sleep" or queryType ==
            "natural_wind" or queryType == "power_saving" or queryType ==
            "fresh_filter_time_total" or queryType == "fresh_filter_time_use" or
            queryType == "fresh_filter_timeout" or queryType ==
            "fresh_filter_timeout_ae2" or queryType == "independent_ptc") then
            for i = 0, 21 do bodyBytes[i] = 0 end
        ...
        elseif (queryType == "group_data_five") then
            ...
            infoM = getTotalMsg(bodyBytes, keyB["BYTE_QUERYL_REQUEST"])
        elseif (queryType == "remote_query") then
            remotequerynum = remotequerynum + 1
            bodyBytes[0] = 0xB0
            bodyBytes[1] = remotequerynum
            -- 远程控制模式
            bodyBytes[2] = 0x02
            bodyBytes[3] = 0x01
            bodyBytes[4] = 0x08
            local remoteControlChannels =
                json["query"]["query_control_channels"]
            -- 获取实际通道数量
            remoteChannels = getChannels(remoteControlChannels)
            -- 计算 bitMask_1 和 bitMask_2
            local bitMask_1 = calculateBitMask(remoteChannels, 0, 31)
            local bitMask_2 = calculateBitMask(remoteChannels, 32, 63)
            extractBytes(bitMask_1, 5, bodyBytes)
            extractBytes(bitMask_2, 9, bodyBytes)
            math.randomseed(tostring(os.time() * #bodyBytes):reverse():sub(1, 7))
            math.random()
            bodyBytes[13] = math.random(1, 254)

            bodyBytes[14] = crc8_854(bodyBytes, 0, 13)
            infoM = getTotalMsg(bodyBytes, 0x02)

        elseif (queryType == "remote_control") then
            remotequerynum = remotequerynum + 1
            bodyBytes[0] = 0xB0
            bodyBytes[1] = remotequerynum
            ...
            for v in values(queryList) do
                ...
                if (queryType == "sup_horizontal_guide_strip_1" or queryType ==
                    "sup_horizontal_guide_strip_2" or queryType ==
                    "sup_horizontal_guide_strip_3" or queryType ==
                    "sup_horizontal_guide_strip_4") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0x31
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x02
                    propertyNum = propertyNum + 1
                end
                ...
                if (queryType == "prevent_straight_wind_select" or queryType ==
                    "prevent_straight_wind_lr") then
                ...
                if (queryType == "continuous_wind") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0x8C
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                if (queryType == "async_wind_swing_lr") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0xBD
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                if (queryType == "out_param") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0xB4
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                if (queryType == "three_dimensional_power_saving") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0x11
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                if (queryType == "power_saving_screen") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0xC0
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                if (queryType == "meeting_mode") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0xC1
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                if (queryType == "control_humidity") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0x7C
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                if (queryType == "prevent_straight_wind_speed") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0xCB
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                if (queryType == "mute") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0xCD
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                if (queryType == "one_control") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0xC2
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                if (queryType == "fan_mode_speed_support") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0xCE
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x00
                    propertyNum = propertyNum + 1
                end
                ...
                if (queryType == "insta_cool") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0x00
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x01
                    propertyNum = propertyNum + 1
                end
                if (queryType == "real_time_power_display") then
                    bodyBytes[1 + propertyNum * 2 + 1] = 0x01
                    bodyBytes[1 + propertyNum * 2 + 2] = 0x01
                    propertyNum = propertyNum + 1
                end
    ...
    elseif (control) then
        ...
        if (control) then
            ...
            if (control["ptc"] ~= nil and
                (control["ptc"] == "on" or control["ptc"] == "off")) then
        ...
        if (keyP["propertyNumber"] == 0) then
            ...
            if (keyP["temperature"] >= 16) then
                bodyBytes[2] = bit.bor(bit.bor(
                                           bit.band(keyP["modeValue"], 0xE0),
                                           bit.band(0x0F, (keyP["temperature"] -
                                                        0x10))), bit.lshift(
                                           bit.band(keyP["smallTemperature"],
                                                    0x01), 4))
            else
                bodyBytes[2] = bit.bor(bit.bor(
                                           bit.band(keyP["modeValue"], 0xE0), 0),
                                       bit.lshift(
                                           bit.band(keyP["smallTemperature"],
                                                    0x01), 4))
            end
            bodyBytes[3] = bit.bor(keyP["fanspeedValue"],
            ...
            bodyBytes[8] = bit.bor(bit.bor(
                                       bit.bor(keyP["strongWindValue"],
                                               keyP["comfortableSleepValue"]),
                                       keyP["power_saving"]),
                                   keyP["follow_body_sense"])
            ...
            if (keyP["temperature_unit"] ~= nil) then
                bodyBytes[10] = bit.bor(bit.lshift(bit.band(
                                                       keyP["temperature_unit"],
                                                       0x01), 2), bodyBytes[10])
            end

            if (keyP["common_filter_reset"] ~= nil) then
                bodyBytes[10] = bit.bor(keyP["common_filter_reset"],
                                        bodyBytes[10])
            end
            ...
            if (keyP["temperature"] < 17 or keyP["temperature"] > 30) then
                if ((keyP["temperature"] >= 13 and keyP["temperature"] < 17) or
                    (keyP["temperature"] <= 38 and keyP["temperature"] > 30)) then
            ...
            if (keyP["comfort_dry_value"] ~= nil) then
                bodyBytes[19] = bit.bor(
                                    bit.band(0x7F, keyP["comfort_dry_value"]),
                                    bodyBytes[19])
            end
            if ((keyP["modeValue"] == keyB["BYTE_MODE_SMART_DRY"] or
                keyP["modeValue"] == keyB["BYTE_MODE_DRY"]) and
            ...
            if (keyP["swingLRUnderSwitch"] ~= nil) then
                bodyBytes[19] = bit.bor(keyP["swingLRUnderSwitch"],
                                        bodyBytes[19])
            end
            if (keyP["swingLRValueUnder"] ~= nil) then
                bodyBytes[20] =
                    bit.bor(keyP["swingLRValueUnder"], bodyBytes[20])
            end
            if (keyP["degree8_heat"] ~= nil) then
                bodyBytes[21] = bit.lshift(bit.band(keyP["degree8_heat"], 0x01),
                                           7)
            end

            if (keyP["comfortPowerSave"] ~= nil) then
                if (keyP["comfortPowerSave"] ==
                    keyB["BYTE_COMFORT_POWER_SAVE_ON"]) then
                    bodyBytes[22] = 0x01
            ...
            if (keyP["fresh_filter_reset"] ~= nil) then
                bodyBytes[22] = bit.bor(keyP["fresh_filter_reset"],
                                        bodyBytes[22])
            end
            ...
            if (keyP["prevent_straight_wind"] ~= nil) then
                ...
                if (keyP["prevent_straight_wind_flag"] ~= nil and
                    keyP["prevent_straight_wind_flag"] == 43) then
            ...
            if (keyP["continuous_wind"] ~= nil) then
                bodyBytes[cursor + 0] = 0x8C
                bodyBytes[cursor + 1] = 0x00
                bodyBytes[cursor + 2] = 0x01
                bodyBytes[cursor + 3] = keyP["continuous_wind"]
                cursor = cursor + 4
            end
            if (keyP["async_wind_swing_lr"] ~= nil) then
                bodyBytes[cursor + 0] = 0xBD
                bodyBytes[cursor + 1] = 0x00
                bodyBytes[cursor + 2] = 0x01
                bodyBytes[cursor + 3] = keyP["async_wind_swing_lr"]
                cursor = cursor + 4
            end
            if (keyP["current_time_year"] ~= nil) then
                bodyBytes[cursor + 0] = 0xBE
                bodyBytes[cursor + 1] = 0x00
                bodyBytes[cursor + 2] = 0x08
                bodyBytes[cursor + 3] =
                    bit.band(keyP["current_time_year"], 0xff)
                bodyBytes[cursor + 4] = bit.band(bit.rshift(
                                                     keyP["current_time_year"],
                                                     8), 0xff)
                bodyBytes[cursor + 5] = keyP["current_time_month"]
                bodyBytes[cursor + 6] = keyP["current_time_week"]
                bodyBytes[cursor + 7] = keyP["current_time_day"]
                bodyBytes[cursor + 8] = keyP["current_time_hour"]
                bodyBytes[cursor + 9] = keyP["current_time_min"]
                bodyBytes[cursor + 10] = keyP["current_time_second"]
                cursor = cursor + 11
            end
            if (keyP["three_dimensional_power_saving"] ~= nil) then
                bodyBytes[cursor + 0] = 0x11
                bodyBytes[cursor + 1] = 0x00
                bodyBytes[cursor + 2] = 0x0C
                bodyBytes[cursor + 3] = keyP["three_dimensional_power_saving"]
                bodyBytes[cursor + 4] =
                    keyP["three_dimensional_power_saving_time"]
                bodyBytes[cursor + 5] = 0
                bodyBytes[cursor + 6] = 0
                bodyBytes[cursor + 7] = 0
                bodyBytes[cursor + 8] = 0
                bodyBytes[cursor + 9] = 0
                bodyBytes[cursor + 10] = 0
                bodyBytes[cursor + 11] = 0
                bodyBytes[cursor + 12] = 0
                bodyBytes[cursor + 13] = 0
                bodyBytes[cursor + 14] = 0
                cursor = cursor + 15
            end
            if (keyP["power_saving_screen"] ~= nil) then
                bodyBytes[cursor + 0] = 0xC0
                bodyBytes[cursor + 1] = 0x00
                bodyBytes[cursor + 2] = 0x01
                bodyBytes[cursor + 3] = keyP["power_saving_screen"]
                cursor = cursor + 4
            end
            if (keyP["meeting_mode"] ~= nil) then
                bodyBytes[cursor + 0] = 0xC1
                bodyBytes[cursor + 1] = 0x00
                bodyBytes[cursor + 2] = 0x01
                bodyBytes[cursor + 3] = keyP["meeting_mode"]
                cursor = cursor + 4
            end
            if (keyP["prevent_straight_wind_speed"] ~= nil) then
                bodyBytes[cursor + 0] = 0xCB
                bodyBytes[cursor + 1] = 0x00
                bodyBytes[cursor + 2] = 0x01
                bodyBytes[cursor + 3] = keyP["prevent_straight_wind_speed"]
                cursor = cursor + 4
            end
            if (keyP["mute"] ~= nil) then
                bodyBytes[cursor + 0] = 0xCD
                bodyBytes[cursor + 1] = 0x00
                bodyBytes[cursor + 2] = 0x01
                bodyBytes[cursor + 3] = keyP["mute"]
                cursor = cursor + 4
            end
            if (keyP["control_humidity"] ~= nil) then
                bodyBytes[cursor + 0] = 0x7C
                bodyBytes[cursor + 1] = 0x00
                bodyBytes[cursor + 2] = 0x02
                bodyBytes[cursor + 3] = keyP["control_humidity"]
                bodyBytes[cursor + 4] = keyP["control_humidity_level"]
                cursor = cursor + 5
            end
            ...
            if (keyP["insta_cool"] ~= nil) then
                bodyBytes[cursor + 0] = 0x00
                bodyBytes[cursor + 1] = 0x01
                bodyBytes[cursor + 2] = 0x01
                bodyBytes[cursor + 3] = keyP["insta_cool"]
                cursor = cursor + 4
            end
            if (keyP["real_time_power_display"] ~= nil) then
                bodyBytes[cursor + 0] = 0x01
                bodyBytes[cursor + 1] = 0x01
                bodyBytes[cursor + 2] = 0x01
                bodyBytes[cursor + 3] = keyP["real_time_power_display"]
                cursor = cursor + 4
            end
            ...
            if (keyP["control_channel"] ~= nil) then
                bodyBytes[cursor + 0] = 0xb1
                bodyBytes[cursor + 1] = 0x00
                if keyP["control_channel"] == 14 then
                    bodyBytes[cursor + 2] = 0x03
                    bodyBytes[cursor + 3] =
                        b1MappingTable[keyP["control_channel"]]
                    for i, j in pairs(remote_control) do
                        bodyBytes =
                            build_bodyBytes_new(i, bodyBytes, cursor + 4)
                    end
                    cursor = cursor + 6
                else
                    bodyBytes[cursor + 2] = 0x02
    ...
    keyP["comfort"] = nil
    keyP["meeting_mode"] = nil
    keyP["b5_meeting_mode"] = nil
    keyP["control_humidity"] = nil
    keyP["control_humidity_level"] = nil
    keyP["b5_control_humidity"] = nil
    keyP["prevent_straight_wind_speed"] = nil
    keyP["cool_power_saving"] = nil
    keyP["b5_cool_power_saving"] = nil
    keyP["degerming"] = nil
...
function dataToJson(jsonCmd)
    ...
    if (keyP["propertyNumber"] == 0) then
        ...
        if (keyP["comfort_dry_value"] ~= nil) then
            streams["comfort_dry_value"] = keyP["comfort_dry_value"]
        end
        -- 净化
        ...
        if ((keyP["outdoorTemperatureValue"] ~= nil) and
            ...
            streams[keyV["VALUE_OUTDOOR_TEMPERATURE"]] =
                keyP["outdoorTemperatureValue"] +
                    keyP["smallOutdoorTemperatureValue"] / 10
        end
        ...
        if ((keyP["PTCValue"] ~= nil) and (keyP["modeValue"] ~= nil)) then
            if (keyP["PTCValue"] == keyB["BYTE_PTC_ON"]) and
                ((keyP["modeValue"] == keyB["BYTE_MODE_AUTO"]) or
                    (keyP["modeValue"] == keyB["BYTE_MODE_HEAT"])) then
        ...
        if (keyP["temperature_unit"] ~= nil) then
            if (keyP["temperature_unit"] == 1) then
                streams[keyT["KEY_TEMPERATURE"]] = convert_to_F(
                                                       keyP["temperature"] +
                                                           streams["small_temperature"])
                streams["small_temperature"] = 0
        ...
        if (keyP["comfortableSleepValue"] ~= nil) then
            if (keyP["comfortableSleepValue"] == 0x00 and
                keyP["comfortableSleepSwitch"] == 0x00) then
                streams["comfort_sleep"] = "off"
            elseif (keyP["comfortableSleepValue"] == 0x03 and
                keyP["comfortableSleepSwitch"] == 0x40) then
        ...
        if (keyP["fresh_filter_timeout_ae2"] ~= nil) then
            streams["fresh_filter_timeout_ae2"] =
                keyP["fresh_filter_timeout_ae2"]
        end
        ...
        if (keyP["outdoor_real_power"] ~= nil) then
            streams["outdoor_real_power"] = keyP["outdoor_real_power"]
        end
        if (keyP["outdoor_system_total_power"] ~= nil) then
            streams["outdoor_system_total_power"] =
                keyP["outdoor_system_total_power"]
        end
        ...
        if (keyP["follow_body_sense"] ~= nil) then
            streams["follow_body_sense"] = keyP["follow_body_sense"]
        end
        ...
        if (keyP["b5_prevent_straight_wind"] ~= nil) then
            streams["b5_prevent_straight_wind"] =
                keyP["b5_prevent_straight_wind"]
        end
        if (keyP["jet_cool"] ~= nil) then
        ...
        if (keyP["anion"] ~= nil) then streams["anion"] = keyP["anion"] end
        if (keyP["b5_anion"] ~= nil) then
            streams["b5_anion"] = keyP["b5_anion"]
        end
        ...
        if (keyP["b5_prevent_straight_wind_select"] ~= nil) then
            streams["b5_prevent_straight_wind_select"] =
                keyP["b5_prevent_straight_wind_select"]
        end
        if (keyP["mito_cool"] ~= nil) then
        ...
        if (keyP["cvp"] ~= nil) then streams["cvp"] = keyP["cvp"] end
        ...
        if (keyP["continuous_wind"] ~= nil) then
            streams["continuous_wind"] = keyP["continuous_wind"]
        end
        if (keyP["b5_continuous_wind"] ~= nil) then
            streams["b5_continuous_wind"] = keyP["b5_continuous_wind"]
        end
        if (keyP["async_wind_swing_lr"] ~= nil) then
            streams["async_wind_swing_lr"] = keyP["async_wind_swing_lr"]
        end
        if (keyP["b5_async_wind_swing_lr"] ~= nil) then
            streams["b5_async_wind_swing_lr"] = keyP["b5_async_wind_swing_lr"]
        end
        if (keyP["b5_request_time"] ~= nil) then
            streams["b5_request_time"] = keyP["b5_request_time"]
        end
        if (keyP["operating_frequency_max"] ~= nil) then
            streams["operating_frequency_max"] = keyP["operating_frequency_max"]
        end
        if (keyP["operating_frequency_min"] ~= nil) then
            streams["operating_frequency_min"] = keyP["operating_frequency_min"]
        end
        if (keyP["inner_operating_max"] ~= nil) then
            streams["inner_operating_max"] = keyP["inner_operating_max"]
        end
        if (keyP["inner_operating_min"] ~= nil) then
            streams["inner_operating_min"] = keyP["inner_operating_min"]
        end
        if (keyP["heat_operating_frequency_max"] ~= nil) then
            streams["heat_operating_frequency_max"] =
                keyP["heat_operating_frequency_max"]
        end
        if (keyP["heat_operating_frequency_min"] ~= nil) then
            streams["heat_operating_frequency_min"] =
                keyP["heat_operating_frequency_min"]
        end
        if (keyP["heat_inner_operating_max"] ~= nil) then
            streams["heat_inner_operating_max"] =
                keyP["heat_inner_operating_max"]
        end
        if (keyP["heat_inner_operating_min"] ~= nil) then
            streams["heat_inner_operating_min"] =
                keyP["heat_inner_operating_min"]
        end
        if (keyP["frequency_point_1"] ~= nil) then
            streams["frequency_point_1"] = keyP["frequency_point_1"]
        end
        if (keyP["frequency_point_2"] ~= nil) then
            streams["frequency_point_2"] = keyP["frequency_point_2"]
        end
        if (keyP["frequency_point_3"] ~= nil) then
            streams["frequency_point_3"] = keyP["frequency_point_3"]
        end
        if (keyP["frequency_point_4"] ~= nil) then
            streams["frequency_point_4"] = keyP["frequency_point_4"]
        end
        if (keyP["frequency_point_5"] ~= nil) then
            streams["frequency_point_5"] = keyP["frequency_point_5"]
        end
        if (keyP["frequency_point_amplitude_1"] ~= nil) then
            streams["frequency_point_amplitude_1"] =
                keyP["frequency_point_amplitude_1"]
        end
        if (keyP["frequency_point_amplitude_2"] ~= nil) then
            streams["frequency_point_amplitude_2"] =
                keyP["frequency_point_amplitude_2"]
        end
        if (keyP["frequency_point_amplitude_3"] ~= nil) then
            streams["frequency_point_amplitude_3"] =
                keyP["frequency_point_amplitude_3"]
        end
        if (keyP["frequency_point_amplitude_4"] ~= nil) then
            streams["frequency_point_amplitude_4"] =
                keyP["frequency_point_amplitude_4"]
        end
        if (keyP["frequency_point_amplitude_5"] ~= nil) then
            streams["frequency_point_amplitude_5"] =
                keyP["frequency_point_amplitude_5"]
        end
        if (keyP["three_dimensional_power_saving"] ~= nil) then
            streams["three_dimensional_power_saving"] =
                keyP["three_dimensional_power_saving"]
        end
        if (keyP["b5_three_dimensional_power_saving"] ~= nil) then
            streams["b5_three_dimensional_power_saving"] =
                keyP["b5_three_dimensional_power_saving"]
        end
        if (keyP["power_saving_screen"] ~= nil) then
            streams["power_saving_screen"] = keyP["power_saving_screen"]
        end
        if (keyP["b5_power_saving_screen"] ~= nil) then
            streams["b5_power_saving_screen"] = keyP["b5_power_saving_screen"]
        end
        if (keyP["three_dimensional_power_saving_time"] ~= nil) then
            streams["three_dimensional_power_saving_time"] =
                keyP["three_dimensional_power_saving_time"]
        end
        if (keyP["control_humidity"] ~= nil) then
            streams["control_humidity"] = keyP["control_humidity"]
        end
        if (keyP["control_humidity_level"] ~= nil) then
            streams["control_humidity_level"] = keyP["control_humidity_level"]
        end
        if (keyP["b5_control_humidity"] ~= nil) then
            streams["b5_control_humidity"] = keyP["b5_control_humidity"]
        end
        if (keyP["meeting_mode"] ~= nil) then
            streams["meeting_mode"] = keyP["meeting_mode"]
        end
        if (keyP["b5_meeting_mode"] ~= nil) then
            streams["b5_meeting_mode"] = keyP["b5_meeting_mode"]
        end
        if (keyP["b5_cool_heat"] ~= nil) then
            streams["b5_cool_heat"] = keyP["b5_cool_heat"]
        end
        if (keyP["prevent_straight_wind_speed"] ~= nil) then
            streams["prevent_straight_wind_speed"] =
                keyP["prevent_straight_wind_speed"]
        end
        if (keyP["b5_prevent_straight_wind_speed"] ~= nil) then
            streams["b5_prevent_straight_wind_speed"] =
                keyP["b5_prevent_straight_wind_speed"]
        end
        if (keyP["mute"] ~= nil) then streams["mute"] = keyP["mute"] end
        if (keyP["b5_mute"] ~= nil) then
            streams["b5_mute"] = keyP["b5_mute"]
        end
        if (keyP["one_control"] ~= nil) then
            streams["one_control"] = keyP["one_control"]
        end
        if (keyP["b5_one_control"] ~= nil) then
            streams["b5_one_control"] = keyP["b5_one_control"]
        end
        if (keyP["one_control_code"] ~= nil) then
            streams["one_control_code"] = keyP["one_control_code"]
        end
        if (keyP["b5_cool_heat_amount"] ~= nil) then
            streams["b5_cool_heat_amount"] = keyP["b5_cool_heat_amount"]
        end
        if (keyP["fan_mode_speed_support"] ~= nil) then
            streams["fan_mode_speed_support"] = keyP["fan_mode_speed_support"]
        end
        if (keyP["b5_fan_mode_speed_support"] ~= nil) then
            streams["b5_fan_mode_speed_support"] =
                keyP["b5_fan_mode_speed_support"]
        end
        if (keyP["cool_power_saving"] ~= nil) then
            streams["cool_power_saving"] = keyP["cool_power_saving"]
        end
        if (keyP["b5_cool_power_saving"] ~= nil) then
            streams["b5_cool_power_saving"] = keyP["b5_cool_power_saving"]
        end
        if (keyP["degerming"] ~= nil) then
        ...
        if (keyP["b5_degerming"] ~= nil) then
            streams["b5_degerming"] = keyP["b5_degerming"]
        end
        if (keyP["b5_prevent_super_cool"] ~= nil) then
            streams["b5_prevent_super_cool"] = keyP["b5_prevent_super_cool"]
        end
        if (keyP["insta_cool"] ~= nil) then
            streams["insta_cool"] = keyP["insta_cool"]
        end
        if (keyP["b5_insta_cool"] ~= nil) then
            streams["b5_insta_cool"] = keyP["b5_insta_cool"]
        end
        if (keyP["real_time_power_display"] ~= nil) then
            streams["real_time_power_display"] = keyP["real_time_power_display"]
        end
        if (keyP["b5_real_time_power_display"] ~= nil) then
            streams["b5_real_time_power_display"] =
                keyP["b5_real_time_power_display"]
        end
        if (keyP["b5_gentle_wind_sense"] ~= nil) then
            streams["b5_gentle_wind_sense"] = keyP["b5_gentle_wind_sense"]
        end
        if (keyP["b5_remote_query"] ~= nil) then
            streams["b5_remote_query"] = keyP["b5_remote_query"]
        end
        if (keyP["b5_remote_control"] ~= nil) then
            streams["b5_remote_control"] = keyP["b5_remote_control"]
        end
        ...
        if (keyP["b5_quick_cool_heat"] ~= nil) then
            streams["b5_quick_cool_heat"] = keyP["b5_quick_cool_heat"]
        end
        if (keyP["imax_wind"] ~= nil) then
            streams["imax_wind"] = keyP["imax_wind"]
        end
        if (keyP["b5_imax_wind"] ~= nil) then
            streams["b5_imax_wind"] = keyP["b5_imax_wind"]
        end
        if (keyP["prevent_super_cool_2"] ~= nil) then
            streams["prevent_super_cool_2"] = keyP["prevent_super_cool_2"]
        end
        if (keyP["b5_prevent_super_cool_2"] ~= nil) then
            streams["b5_prevent_super_cool_2"] = keyP["b5_prevent_super_cool_2"]
        end
        -- 远程控制模式所有属性
        for i, j in pairs(remote_query) do
        ...
        if keyP["unchange_value_channel_0e"] ~= nil then
            for i = 1, keyP["unchange_value_channel_0e"] do
                streams["channel_" .. i] = keyP["channel_" .. i]
                streams["channel_value_" .. i] = keyP["channel_value_" .. i]
            end
        end
    end
    keyP["propertyNumber"] = 0
