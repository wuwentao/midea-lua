...
local function jsonToModel(controlJson)
    ...
    elseif (myTable["controlType"] == 0x08) then
        ...
        if controlCmd["sensor_temp_heating_on_hour"] ~= nil then
            myTable["sensor_temp_heating_on_hour"] =
                controlCmd["sensor_temp_heating_on_hour"]
        end
        if controlCmd["sensor_temp_heating_on_min"] ~= nil then
            myTable["sensor_temp_heating_on_min"] =
                controlCmd["sensor_temp_heating_on_min"]
        end
        ...
        if controlCmd["dynamic_night_power_on_hour"] ~= nil then
            myTable["dynamic_night_power_on_hour"] =
                controlCmd["dynamic_night_power_on_hour"]
        end
        if controlCmd["dynamic_night_power_on_min"] ~= nil then
            myTable["dynamic_night_power_on_min"] =
                controlCmd["dynamic_night_power_on_min"]
        end
        if controlCmd["dynamic_night_power_off_hour"] ~= nil then
            myTable["dynamic_night_power_off_hour"] =
                controlCmd["dynamic_night_power_off_hour"]
        end
        if controlCmd["dynamic_night_power_off_min"] ~= nil then
            myTable["dynamic_night_power_off_min"] =
                controlCmd["dynamic_night_power_off_min"]
        end
...
function jsonToData(jsonCmd)
    ...
    elseif (control) then
        ...
        if (myTable["controlType"] == 0x01) then
            ...
            -- 新增静音
            bodyBytes[8] =
                bit.bor(bit.band(myTable["mute"], 0x08), bodyBytes[8])
        ...
        elseif (myTable["controlType"] == 0x02) then
            ...
            bodyBytes[3] = bit.bor(bit.band(myTable["timer1Effect"], 0x01),
                                   ...
                                   bit.band(myTable["single_timer_on"], 0x40),
                                   bit.band(myTable["single_timer_off"], 0x80))
...
function dataToJson(jsonCmd)
    ...
    elseif (msgSubType == 0xB0 or msgSubType == 0xB5 or msgSubType == 0xB1) then
        ...
        if (myTable["sensor_temp_heating_on_hour"] ~= nil) then
            streams["sensor_temp_heating_on_hour"] =
                myTable["sensor_temp_heating_on_hour"]
        end
        if (myTable["sensor_temp_heating_on_min"] ~= nil) then
            streams["sensor_temp_heating_on_min"] =
                myTable["sensor_temp_heating_on_min"]
        end
        if (myTable["dynamic_night_power"] ~= nil) then
        ...
        if (myTable["sterilize_effect_enable"] ~= nil) then
            streams["sterilize_effect_enable"] =
                myTable["sterilize_effect_enable"]
        end
        if (myTable["appointment_timer"] ~= nil) then
