...
local function binToModel(binData)
    ...
    local messageBytes = {}
    for i = 0, 29 do messageBytes[i] = 0 end
    ...
    if (myTable["dataType"] == 0x03 or myTable["dataType"] == 0x05) then
        ...
        myTable["compatibilizingMode"] = bit.rshift(
                                             bit.band(messageBytes[2], 0x08), 3)
        ...
        myTable["fourWayValve"] =
            bit.rshift(bit.band(messageBytes[27], 0x20), 5)
        myTable["lowWind"] = bit.rshift(bit.band(messageBytes[27], 0x40), 6)
        myTable["highWind"] = bit.rshift(bit.band(messageBytes[27], 0x80), 7)
        myTable["timer1Effect"] =
            bit.rshift(bit.band(messageBytes[28], 0x02), 1)
        myTable["timer2Effect"] =
            bit.rshift(bit.band(messageBytes[28], 0x04), 2)
        myTable["order1Effect"] =
            bit.rshift(bit.band(messageBytes[28], 0x08), 3)
        myTable["order2Effect"] =
            bit.rshift(bit.band(messageBytes[28], 0x10), 4)
        myTable["smartEffect"] = bit.rshift(bit.band(messageBytes[28], 0x20), 5)
        myTable["backwaterEffect"] = bit.rshift(
                                         bit.band(messageBytes[28], 0x40), 6)
        myTable["sterilizeEffect"] = bit.rshift(
                                         bit.band(messageBytes[28], 0x80), 7)
    ...
    elseif myTable["dataType"] == 0x02 then
        ...
        if myTable["controlType"] == 0x01 then
            ...
            myTable["openPTCTemp"] = bit.rshift(bit.band(messageBytes[8], 0x02),
                                                6)
        elseif (myTable["controlType"] == 0x02) then
            myTable["timer1Effect"] = bit.band(messageBytes[3], 0x01)
            myTable["timer2Effect"] = bit.rshift(
                                          bit.band(messageBytes[3], 0x02), 1)
...
function jsonToData(jsonCmd)
    ...
    elseif (control) then
        ...
        if (myTable["controlType"] == 0x01) then
            for i = 0, 8 do bodyBytes[i] = 0 end
            bodyBytes[0] = 0x01
            bodyBytes[1] = 0x01
            ...
            bodyBytes[8] = bit.bor(bit.bor(bit.bor(
                                               bit.bor(myTable["waterPump"],
                                                       myTable["refrigerantRecycling"]),
                                               myTable["defrost"]),
                                           myTable["mute"]),
                                   myTable["openPTCTemp"])
        ...
        elseif (myTable["controlType"] == 0x02) then
            ...
            bodyBytes[1] = 0x01
            bodyBytes[2] = 0x02
            bodyBytes[3] = bit.bor(myTable["timer1Effect"],
                                   myTable["timer2Effect"], 0, 0)
            ...
            bodyBytes[13] = 45;
            for i = 14, 23 do bodyBytes[i] = 0 end
            infoM = getTotalMsg(bodyBytes, BYTE_CONTROL_REQUEST)
        elseif (myTable["controlType"] == 0x03) then
            ...
            bodyBytes[3] = int2String(
                               math.modf((myTable["order1Temp"] * 2) + 30))
            bodyBytes[4] = myTable["order1TimeHour"]
            bodyBytes[5] = myTable["order1TimeMin"]
            bodyBytes[6] = myTable["order2Effect"]
            bodyBytes[7] = int2String(
                               math.modf((myTable["order2Temp"] * 2) + 30))
...
function dataToJson(jsonCmd)
    ...
    if (((myTable["dataType"] == BYTE_AUTO_REPORT) and (msgSubType == 0x01)) or
        (myTable["dataType"] == BYTE_QUERYL_REQUEST)) then
        ...
        streams["set_temperature"] = int2String(math.modf(
                                                    (myTable["tsValue"] - 30) /
                                                        2))
        streams["water_box_temperature"] =
            int2String(math.modf((myTable["washBoxTemp"] - 30) / 2))
        streams["water_box_top_temperature"] =
            int2String(math.modf((myTable["boxTopTemp"] - 30) / 2))
        streams["water_box_bottom_temperature"] =
            int2String(math.modf((myTable["boxBottomTemp"] - 30) / 2))
        streams["condensator_temperature"] =
            int2String(math.modf((myTable["t3Value"] - 30) / 2))
        streams["outdoor_temperature"] =
            int2String(math.modf((myTable["t4Value"] - 30) / 2))
        streams["compressor_top_temperature"] =
            int2String(math.modf((myTable["compressorTopTemp"] - 30) / 2))
        streams["set_temperature_max"] =
            int2String(math.modf((myTable["tsMaxValue"] - 30) / 2))
        streams["set_temperature_min"] =
            int2String(math.modf((myTable["tsMinValue"] - 30) / 2))
        ...
        streams["order1_temp"] = int2String(math.modf(
                                                (myTable["order1Temp"] - 30) / 2))
        streams["order1_timeHour"] = int2String(myTable["order1TimeHour"])
        streams["order1_timeMin"] = int2String(myTable["order1TimeMin"])
        streams["order2_temp"] = int2String(math.modf(
                                                (myTable["order2Temp"] - 30) / 2))
